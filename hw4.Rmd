...
Q7. So far you have considered a single type of model. Let's try to perform this classification using a simple single-layer neural network from the `keras` package. The documentation for `parsnip` (part of `tidymodels`) explains how to implement a neural network using the `mlp` function: https://parsnip.tidymodels.org/reference/mlp.html

Complete the code below to specify the model and parameter grid. The model should have the following components: the number of hidden units and dropout should be tunable, each with two levels. For the number of units, test three different values between 10 and 100. For dropout, evaluate models with values of 0 and 0.05. The training epochs should be fixed to 100. No additional arguments need to be specified.
